...
class SKUOutboundService {
    constructor(beSKUInboundService, beSKUOutboundService, beService) {
        this.beSKUInboundService = beSKUInboundService;
        this.beSKUOutboundService = beSKUOutboundService;
        this.beService = beService;
    }

    async sendOut(skuOutboundID, status, warehouse) {
        try {
            // Get SKU_Outbound by ID
            let skuOutbound = await this.beSKUOutboundService.findOneByID(skuOutboundID);
            // console.log(skuOutbound);
            ...
            if (
                _.get(skuOutbound, "LINK_SKUMatching.objectId") == null ||
                _.get(skuOutbound, "LINK_SKUMatching.objectId") == undefined
            ) {
                throw new Error("sku_inbound_notfound")
            }

            let skuInbounds = await this.beSKUInboundService.getAllForSendOut(
                _.get(skuOutbound, "LINK_SKUMatching.objectId"),
                warehouse,
            )
            ...
            const payloads = this.createBEPayloadsToUpdateQtyToBeSentOut(skuOutbound, skuInbounds, status);
    ...
    createBEPayloadsToUpdateQtyToBeSentOut(skuOutbound, skuInbounds, status) {
        let payloads = [];

        let qtyLeft = skuOutbound['Qty'];
        // console.log("Initial: ", qtyLeft)

        let skuInboundStatusLOCRecords = [];
        let skuOutboundStatusLOCRecords = [];

        if (skuOutbound["STATUS_LOC_Records"] != null) {
            skuOutboundStatusLOCRecords = JSON.parse(skuOutbound["STATUS_LOC_Records"]);
        }
        ...
        for(let i = 0; i < skuInbounds.length; i++) {
            ...
            let skuInboundMetadata = {
                "DateTime": new Date(),
                "sku_outbound_objectid": skuOutbound.objectId,
            }
            ...
            if(
                ...
                _.merge(skuInboundMetadata, {
                    "Old_CurrentLocation": skuOutbound.CurrentLocation,
            ...
            skuOutboundStatusLOCRecords.push({
                "DateTime": new Date(),
        ...
        payloads.push({
            "table": BACKENDLESS.TABLE.SKUOutbound,
            "id": skuOutbound.objectId,
            "body": {
                "Status": status,
                "STATUS_LOC_Records": JSON.stringify(skuOutboundStatusLOCRecords),
            },
        })
